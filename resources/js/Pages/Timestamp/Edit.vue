...
const startTimeHour = ref(Math.floor(parseInt(props.timestamp.started_at.formatted) / 100))
const startTimeMinute = ref(Math.floor(parseInt(props.timestamp.started_at.formatted) % 100))

const endTimeHour = ref(Math.floor(parseInt(props.timestamp.ended_at?.formatted ?? '0') / 100))
const endTimeMinute = ref(Math.floor(parseInt(props.timestamp.ended_at?.formatted ?? '0') % 100))
    ...
    >
        <template #title>
            <div class="flex items-center gap-2">
                ...
                {{
                    $t('app.edit :item', {
                        item: $t(props.timestamp.type === 'work' ? 'app.work hours' : 'app.break')
                    })
                }}
            </div>
        </template>
                ...
                <div class="mt-4 flex flex-1 gap-2">
                    ...
                    <div class="w-20 shrink-0 text-right font-medium">
                        {{
                            $t('app.:time', {
                                time: moment(
                                    secToFormat(startTimeHour * 3600 + startTimeMinute * 60, false, true, true),
                                    'HH:mm'
                                ).format('LT')
                            })
                        }}
                ...
                <div class="mt-4 flex flex-1 gap-2">
                    ...
                    <div class="w-20 shrink-0 text-right font-medium">
                        {{
                            $t('app.:time', {
                                time: moment(
                                    secToFormat(endTimeHour * 3600 + endTimeMinute * 60, false, true, true),
                                    'HH:mm'
                                ).format('LT')
                            })
                        }}
